...
void makeAnnotation(const int n_label, const QString img_path,const QString unary_path,const QString png_path)
{
  shotton::EnergyParam energy_param;
  shotton::train("VOC", &energy_param);
  Eigen::MatrixXi ann = shotton::annotate(n_label,qPrintable(img_path), qPrintable(unary_path), energy_param);
  png_write(ann,qPrintable(png_path));
}
