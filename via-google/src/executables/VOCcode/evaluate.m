...
function [accuracies,avacc,conf,rawcounts] = VOCevalseg(groundtruth_folder,prediction_folder,testlist)
VOCinit

%CONFIG
%groundtruth_folder='/home/rizkivmaster/ws/datasets/miniVOC2010/SegmentationClass';
groundtruth_path=[groundtruth_folder '/%s.png'];
%prediction_folder='/home/rizkivmaster/ws/results/svm-dump-600-normal-test';
prediction_path=[prediction_folder '/%s.png'];
%test_list='/home/rizkivmaster/ws/datasets/voc2010/split/Test.txtOri';

% image test set
[gtids,t]=textread(test_list,'%s %d');
...
for i=1:length(gtids)
    ...
    gtfile = sprintf(groundtruth_path,imname);
    [gtim,map] = imread(gtfile);    
    gtim = double(gtim);
    
    % results file
    resfile = sprintf(prediction_path,imname);
