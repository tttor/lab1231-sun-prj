...
SAMPLE      read_struct_examples(char *file, STRUCT_LEARN_PARM *sparm)
{
  ...
  if(reader.is_open())
  {
    ...
    {
      string targetunarypath = ssvm_ss::dataset::unary_directory+"/" + id + ".unary";
      string targetpngpath = ssvm_ss::dataset::png_directory+"/" + id + ".png";
      string targetimagepath = ssvm_ss::dataset::jpg_directory+"/" + id + ".jpg";


      __(targetunarypath.c_str(),3);
      //read unary
      ProbImage unary_matrix;
      unary_matrix.load( targetunarypath.c_str() );
      __("PART ",5);
      //read png
      QImage png_matrix;
      png_matrix.load(targetpngpath.c_str(),"png");
      __("PART ",6);
      //read jpg
      const cv::Mat image_matrix = cv::imread(targetimagepath, CV_LOAD_IMAGE_COLOR);

      __("cols",image_matrix.cols);
      //assign to examples
      examples[exampleIndex].x.height = png_matrix.height();
      examples[exampleIndex].x.width = png_matrix.width();
      __("PART",examples[exampleIndex].x.height);
      examples[exampleIndex].x.unary_matrix = unary_matrix;
      __(targetimagepath.c_str(),10);
      examples[exampleIndex].x.image_matrix = (cv::Mat*)&image_matrix;
      printf("image address : %d \n",examples[exampleIndex].x.image_matrix);
      __("PART",11);
      ...
      printf("image width %d %d \n",examples[exampleIndex].x.image_matrix->cols,examples[exampleIndex].x.image_matrix->rows);
...
void set_pair_weights(STRUCTMODEL *sm, PATTERN x,double* weights)
{
  ...
  {
    //set target png
    QImage targetPNG(x.width,x.height,QImage::Format_Indexed8);
    y.png_matrix = targetPNG;
    y.height = x.height;
    y.width = x.width;
    y.n_label = ssvm_ss::image_constraint::n_label;
    printf("image width %d %d \n",x.image_matrix->cols,x.image_matrix->rows);
    //set weights
    double unary_weights[x.width*x.height];
    double pair_weights[(x.width-1)*(x.height-1)];
    set_unary_weights(sm,x,unary_weights);
    set_pair_weights(sm,x,pair_weights);
    printf("image address : %d \n",x.image_matrix);
    printf("image_size %d %d\n",x.image_matrix->cols,x.image_matrix->rows);
    lab1231_sun_prj::shotton::annotate(y.n_label,x.image_matrix,x.unary_matrix,unary_weights,pair_weights,y.png_matrix);
  //set up inferer
...
_("start init psi");
WORD * words = (WORD*) my_malloc(sizeof(WORD)*sm->sizePsi);
__("PSI",1);
for(size_t indexPsi = 0 ; indexPsi < sm->sizePsi;indexPsi++){
  words[indexPsi].wnum = indexPsi+1;
  words[indexPsi].weight = 0.0;
}
...
for(size_t xx = 0; xx < x.width; xx++)
  for(size_t yy = 0; yy < x.height;yy++)
  {
    ...
    words[(windowoffsetx+xx)+(windowoffsety+yy)*windowwidth].weight = x.unary_matrix(xx,yy,y.png_matrix.pixelIndex(xx,yy));
  }

__("PSI",4);
assert(sm->sizePsi - windowoffset >0);
__("size psi",sm->sizePsi - windowoffset);
  double * pair_psi = (double*)malloc(sizeof(double)*(sm->sizePsi - windowoffset));
  __("PSI",7);
  lab1231_sun_prj::shotton::get_2nd_order_psi(x.image_matrix,y.n_label,x.unary_matrix,y.png_matrix,pair_psi);
  __("PSI",8);
  ...
  for(size_t xx = 0; xx < x.width-1; xx++)
    ...
    {
      words[windowoffset+(windowoffsetx+xx)+(windowoffsety+yy)*windowwidth].weight = pair_psi[xx+(yy*(x.width-1))];
      words[windowoffset+type1psioffset+(windowoffsetx+xx)+(windowoffsety+yy)*windowwidth].weight = pair_psi[type1psioffset+xx+(yy*(x.width-1))];
    }
    __("PSI",6);
...
_("end init psi");
  ...
  if(sparm->loss_function == 0) { /* type 0 loss: 0/1 loss */
    ...
    for(int xx = 0; xx < y.width;xx++)
      ...
      {
        sum += (y.png_matrix.pixelIndex(xx,yy)!=ybar.png_matrix.pixelIndex(xx,yy));
      }
      return sum;
    }
    else {
