...
def calc_coverage_score(chromosome):
    ...
    total_pixel_in_segment = len(set(pixel_in_segment))
    total_pixel_in_image = len(img) * len(img[0])
    return float(total_pixel_in_segment) / total_pixel_in_image

def calc_probability_score(chromosome):
    global index_converter
    ind_bit_1 = [ind for ind, bit in zip(range(len(chromosome)),chromosome) if bit == 1]
    total_prob_in_segment = sum([index_converter[ind][1][0] for ind in ind_bit_1])    
    return float(total_prob_in_segment) / len(ind_bit_1)

def calc_nonoverlap_score(chromosome):
    global index_converter
    ind_bit_1 = [ind for ind, bit in zip(range(len(chromosome)),chromosome) if bit == 1]
    pixel_in_segment = []
    for ind in ind_bit_1:
...
def eval_func(chromosome):
    coverage_score = calc_coverage_score(chromosome)
    nonoverlap_score = calc_nonoverlap_score(chromosome)
    probability_score = calc_probability_score(chromosome)
